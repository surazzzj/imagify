    ...
    return (
        <div className='fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
            <motion.form onSubmit={onSubmitHandler}
                ...
                {state !== 'Login' && <div className='border border-gray-200 px-6 py-2 flex items-center gap-2 rounded-full mt-5'>
                    <img width={20} className='opacity-[40%]' src={assets.user_icon} alt="" />
                    <input value={name} onChange={(e) => setName(e.target.value)} className='outline-none text-sm' type="text" placeholder='Full Name' required />
                </div>}
                <div className='border border-gray-200  px-6 py-2 flex items-center gap-2 rounded-full mt-4'>
                    ...
                    <input value={email} onChange={(e) => setEmail(e.target.value)} className='outline-none text-sm' type="email" placeholder='Email' required />
                </div>
                <div className='border border-gray-200  px-6 py-2 flex items-center gap-2 rounded-full mt-4'>
                    <img width={13} src={assets.lock_icon} alt="" />
                    <input value={password} onChange={(e) => setPassword(e.target.value)} className='outline-none text-sm' type="password" placeholder='Password' required />
                </div>
